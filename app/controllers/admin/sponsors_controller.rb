class Admin::SponsorsController < AdminController
  ...
  # GET /sponsors.json
  def index
    @sponsors = Sponsor.all.order('posnr ASC')
  end
  ...
  def create
    ...
    respond_to do |format|
      if @sponsor.save
        ...
        format.json { render :show, status: :created, location: [:admin, @sponsor] }
      else
  ...
  def update
    respond_to do |format|
      if @sponsor.update(sponsor_params)
        ...
        format.json { render :show, status: :ok, location: [:admin, @sponsor] }
      else
  ...
  private
    ...
    def sponsor_params
      params.require(:sponsor).permit(:posnr, :name, :logo, :visible)
    end
end
